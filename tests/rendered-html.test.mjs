...
test("medication payment holds the order for one hour with one persistent countdown", async () => {
  ...
  assert.match(html, /data-payment-deadline-retry[\s\S]*invalidateFulfillmentQuote\(\)[\s\S]*show\('pharmacy-search'\)/, "expiry must offer a fresh stock and price check");
  assert.match(css, /\.payment-deadline\{[\s\S]*\.payment-deadline__time/);
