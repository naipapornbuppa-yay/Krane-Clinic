(() => {
  const translations = {
    en: {
      ...
      reviewsMembers: "members and counting",
  ...
  const revealGroups = [
    ...
    [".compliance__logos > div", 55, "scale"],
    [".steps > li", 75, ""],
  ...
  const productSlides = [
    {
      src: "assets/landing-573/products/clinical-pens.png",
      alt: "ตัวอย่างผลิตภัณฑ์การรักษาที่แพทย์อาจสั่งจ่าย"
    },
    {
      src: "assets/figma-296-3852/protocol-hair.png",
      alt: "ตัวอย่างโปรโตคอลดูแลเส้นผม"
    },
    {
      src: "assets/figma-296-3852/protocol-sexual.png",
      alt: "ตัวอย่างโปรโตคอลดูแลสุขภาพทางเพศ"
    }
  ];
  let activeProduct = 0;
  ...
  function renderProduct() {
    ...
    productSwapTimer = window.setTimeout(() => {
      productImage.src = productSlides[activeProduct].src;
      productImage.alt = productSlides[activeProduct].alt;
  ...
  let reviewLoopWidths = [];
  let reviewMobileLoopWidth = 0;
  let reviewLayoutMode = "";
  ...
  if (reviewCollage && originalReviewCards.length) {
    reviewColumns.forEach((column) => {
      [...column.querySelectorAll(":scope > [data-review-open]")].forEach((card) => {
        column.append(createReviewClone(card, originalReviewCards.indexOf(card), "vertical"));
      });
    });
    ...
    function measureReviewLoops() {
      ...
      reviewLoopWidths = reviewColumns.map((column) => {
        const original = column.querySelector(":scope > [data-review-open]");
        const clone = column.querySelector(":scope > .review-loop-clone--vertical");
        return original && clone ? Math.max(0, clone.offsetTop - original.offsetTop) : 0;
      });
      const firstOriginal = originalReviewCards[0];
      const firstMobileClone = mobileSequence.querySelector(".review-loop-clone--mobile");
      reviewMobileLoopWidth = firstOriginal && firstMobileClone
        ? Math.max(0, firstMobileClone.offsetLeft - firstOriginal.offsetLeft)
        : 0;

      if (reviewLayoutMode !== mode) {
        reviewLayoutMode = mode;
        reviewCollage.scrollLeft = 0;
        reviewColumns.forEach((column, index) => {
          const loop = reviewLoopWidths[index] || 0;
          column.scrollTop = column.classList.contains("review-column--reverse")
            ? Math.max(0, loop - 116)
            : index === 1 ? Math.min(6, loop) : 0;
        });
      }
    }
    ...
    function syncReviewMotionPreference() {
      ...
      if (isStatic) {
        reviewCollage.scrollLeft = 0;
        reviewColumns.forEach((column) => { column.scrollTop = 0; });
      }
      reviewLayoutMode = "";
    ...
    function animateReviews(time) {
      ...
      if (!paused && elapsed > 0) {
        if (window.matchMedia("(max-width: 700px)").matches) {
          if (reviewMobileLoopWidth > 0) {
            reviewCollage.scrollLeft += elapsed * 0.025;
            if (reviewCollage.scrollLeft >= reviewMobileLoopWidth) {
              reviewCollage.scrollLeft -= reviewMobileLoopWidth;
            }
          }
        } else {
          reviewColumns.forEach((column, index) => {
            const loop = reviewLoopWidths[index] || 0;
            if (!loop) return;
            const reverse = column.classList.contains("review-column--reverse");
            column.scrollTop += elapsed * (reverse ? -0.018 : 0.018);
            if (reverse && column.scrollTop <= 0) column.scrollTop += loop;
            if (!reverse && column.scrollTop >= loop) column.scrollTop -= loop;
          });
        }
      }
